...
void chronologicalMemoryModelDelayRescued(
    ...
    //Specific variable to take into account modification date with delay
    std::vector<int> startFruitingDateSpeciesOperationalTrue;
    std::vector<int> endFruitingDateSpeciesOperationalTrue;
    for(int fr=0; fr<numberSpecies; fr++){
        //Put already to one those that have started fruiting before beginning of the year
        startFruitingDateSpeciesOperationalTrue.push_back(floor(startFruitingDateSpeciesOperational[fr]-delay - 0.625/(double) sqrt(numberIndividualsPerSpecies)*(1 - intraSpeciesSynchrony[fr])*cycleLength/2));
        endFruitingDateSpeciesOperationalTrue.push_back(floor(endFruitingDateSpeciesOperational[fr]-delay + 0.625/(double) sqrt(numberIndividualsPerSpecies)*(1 - intraSpeciesSynchrony[fr])*cycleLength/2));
    }

//    std::vector<int> speciesSeenFruitingDuringYear=howManyTimesSpeciesSeenFruiting;
    std::vector<int> hasPassedOneFruiting(numberSpecies,0);
    ...
    while(timer <cycleLength){//latestEndFruitingDate){
            ...
            if(fmod(10*cycleLength + timer, cycleLength) > fmod(10*cycleLength + endFruitingDateSpeciesOperational[s] + fruitingLength, cycleLength)){
               hasPassedOneFruiting[s]=1;
            }
                //Calculate food inference
...
//**UPDATE ABILITIES
                    ...
                    numberVisitedTreesForSpecies[speciesTree[t]]=numberVisitedTreesForSpecies[speciesTree[t]]+1;
                     if(foodQuantityTree[t]>0.00001){//If food
                            //Adjust known date for second time (i.e. when recording foraging success)
                            if((fmod(10*cycleLength + startFruitingDateSpeciesOperational[speciesTree[t]], cycleLength) > fmod(10*cycleLength + timer, cycleLength))&&
                               (fmod(10*cycleLength + startFruitingDateSpeciesOperational[speciesTree[t]] - abs(delay), cycleLength) < fmod(10*cycleLength + timer, cycleLength))&&
                               (hasPassedOneFruiting[speciesTree[t]]==1)
                               ){
                                    int diffStartEnd(fmod(10*cycleLength + endFruitingDateSpeciesOperational[speciesTree[t]] - startFruitingDateSpeciesOperational[speciesTree[t]], cycleLength));
                                    startFruitingDateSpeciesOperational[speciesTree[t]]= floor(timer);
                                    endFruitingDateSpeciesOperational[speciesTree[t]]= floor(timer + diffStartEnd);
